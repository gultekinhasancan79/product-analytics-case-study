...
def pre_treatment_activation_score(row: dict[str, object]) -> float:
    ...
    This score excludes the randomized variant and all post-exposure outcomes.
    In production it would be preferable to freeze the covariate before launch.
    """
...
def cuped_adjust_activation(rows: list[dict[str, object]]) -> CupedResult:
    ...
    new-user onboarding experiment has no genuine pre-period activation metric,
    so the demonstration uses a treatment-blind score constructed only from
    pre-exposure device and acquisition-channel fields. The unadjusted primary
    analysis remains the confirmatory result.
    """
    if len(rows) < 4:
