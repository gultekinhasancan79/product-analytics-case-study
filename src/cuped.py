...
def pre_treatment_activation_score(row: dict[str, object]) -> float:
    """Treatment-blind activation propensity from pre-exposure dimensions."""
...
def cuped_adjust_activation(rows: list[dict[str, object]]) -> CupedResult:
    ...
    Classical CUPED often uses a pre-period version of the outcome. This is a
    new-user onboarding experiment, so no genuine pre-period activation measure
    exists. The demonstration therefore uses only treatment-blind device and
    acquisition-channel information. The unadjusted primary analysis remains
    the confirmatory decision statistic.
    """
    if len(rows) < 4:
    ...
    raw_control = [
        ...
        if str(row["variant"]) == "control"
    ]
    raw_treatment = [
        outcome
        for outcome, row in zip(outcomes, rows)
        if str(row["variant"]) == "treatment"
    ]
    adjusted_control = [
        value
        for value, row in zip(adjusted, rows)
        if str(row["variant"]) == "control"
    ]
    adjusted_treatment = [
        value
